...
class DiscriminantANoyau:
    ...
    def fit(self, X, y):
        ...
        def evaluateFunc(hypers):
            ...
            gradalpha = -y*numpy.dot(
                numpy.array(y[ind_mal_classe]),
                numpy.array(Kgauss[ind_mal_classe, :])) +\
                self.lambda_
        ...
        params, minval, infos = fmin_l_bfgs_b(
            evaluateFunc, params, bounds=bounds, factr=1e13, epsilon=2)
        _times.append(time.time())
...
if __name__ == "__main__":
    ...
    listSigma = [0.03*x for x in range(50//3, 90//3)]
    listLambda = [0.1*x for x in range(1, 10)] + [0.1, 0.15, 0.2]
    listDaN = []
    ...
    tps_ent.append(time.time())
    print("entrainement en ", tps_ent[-1]-tps_ent[-2])
    # for k, v in ErrorDaN.items():
    #     print(k, v)
    print("argmin Lambda, Sigma = ", argMinDaN)
    print("minErrorDaN", minErrorDaN)
    GoodParameters = [k for k, v in ErrorDaN.items() if v < 0.10]
    print("Good parameters", GoodParameters)

    fig, sfig = pyplot.subplots(1, 1, sharex=True, sharey=True)
    y_label = [str(list(ErrorDaN.keys())[i][0])
               for i in range(len(list(ErrorDaN.keys())))],
    x_values = [list(ErrorDaN.keys())[i][1]
                for i in range(len(list(ErrorDaN.keys())))]
    ...
    print(len(y_label))
    print(len(x_values))
    err = sfig.scatter(y_label[0], x_values, s=60,
                       c=list(ErrorDaN.values()), alpha=0.8)
    sfig.set_xlabel("lambda")
    sfig.set_ylabel("Sigma")
    pyplot.colorbar(err, format='Err %.2f')
    sfig.xaxis.set_major_locator(pyplot.MaxNLocator(3))
    pyplot.show()
